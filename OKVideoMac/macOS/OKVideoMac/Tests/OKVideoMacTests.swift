...
final class OKVideoMacTests: XCTestCase {
    ...
    func testNodeAuthorizationContinuationRequiresNewCredentialForSameSession() {
        XCTAssertTrue(
            NodeAuthorizationContinuationPolicy.shouldContinue(
                status: "CONFIRMED",
                baselineCredentialRevision: "revision-before",
                currentCredentialRevision: "revision-after",
        ...
        XCTAssertFalse(
            NodeAuthorizationContinuationPolicy.shouldContinue(
                status: "CONFIRMED",
                baselineCredentialRevision: "revision-before",
                currentCredentialRevision: "revision-before",
                boundSessionID: "session-1",
                returnedSessionID: "session-1",
                alreadyStarted: false
            )
    ...
    func testNodeAuthorizationContinuationIsSingleShotAndWaitsForConfirmation() {
        XCTAssertFalse(
            NodeAuthorizationContinuationPolicy.shouldContinue(
                status: "RETRYING",
                baselineCredentialRevision: nil,
                currentCredentialRevision: "revision-after",
                boundSessionID: "session-1",
                returnedSessionID: "session-1",
                alreadyStarted: false
...
final class NodeBundleCompatibilityTests: XCTestCase {
    ...
    func testNodeProviderTreatsHTTP200EmptyBDUSSResultAsAuthorization() async throws {
        let site = SiteConfiguration(
            key: "nodejs_baidu",
            name: "Cloud Source",
            type: 4,
        ...
        let client = NodeProviderStubHTTPClient { request in
            ...
            if request.url.path.hasSuffix(
                ...
                return HTTPResponse(
                    ...
                    headers: ["Content-Type": "application/json"],
                    body: Data(
                        #"{"code":0,"data":{"status":"IDLE","credentialRevision":"before-login"}}"#.utf8
                    )
                )
            }
            ...
            return HTTPResponse(
                ...
                statusCode: 200,
                headers: ["Content-Type": "application/json"],
                body: Data(#"{"url":"","msg":"BDUSS为空"}"#.utf8)
            )
        }
        let provider = try NodeHTTPSpiderSiteProvider(
            site: site,
        ...
        do {
            _ = try await provider.player(flag: "百度", episodeURL: "opaque")
            XCTFail("HTTP 200 的空 BDUSS 结果不得进入播放器")
        } catch let authorization as NodeWebAuthorizationRequired {
            XCTAssertEqual(authorization.providerID, "baidu")
            XCTAssertEqual(
                authorization.baselineCredentialRevision,
                "before-login"
            )
            XCTAssertEqual(
                authorization.authorizationStatusURL?.path,
                "/website/baidu/authorization-state"
            )
        }
    }

    func testKnownDouerBundleGetsIsolatedBaiduRuntimePatch() throws {
        let sourceText =
            "before;d(),p.current=setInterval(async()=>{work()"
                + "}catch(I){}},2e3),m.current=timer;"
                + "await E.put(t,{cookie:N});"
                + "var C40=qe(J6(),1),N40=qe(oa(),1);var bE=legacy;"
                + "var Wqt=require(\"crypto\");after"
        let source = Data(sourceText.utf8)
        let url = try XCTUnwrap(URL(
            string: "https://raw.githubusercontent.com/Darklessing/catvod/refs/heads/main/douer/index.js"
        ))

        let patched = try XCTUnwrap(
            NodeBundleRuntimeCompatibilityPatcher.patchedScript(
                source,
                sourceURL: url
            )
        )
        ...
        XCTAssertTrue(text.hasSuffix("var Wqt=require(\"crypto\");after"))
        XCTAssertTrue(text.contains("/authorization-state"))
        XCTAssertTrue(text.contains("credential_revision"))
        XCTAssertTrue(text.contains("pollPromise"))
        XCTAssertTrue(text.contains("p.current=setTimeout(R,0)"))
        XCTAssertTrue(text.contains("N&&await E.put(t,{cookie:N})"))
        XCTAssertFalse(text.contains("p.current=setInterval(async"))
        XCTAssertFalse(text.contains("var bE=legacy"))
        XCTAssertEqual(
            String(data: source, encoding: .utf8),
            sourceText
        )
    }

    func testStrictNodeMediaReadinessRejectsHTTP200JSONFallback() async throws {
        let mediaURL = try XCTUnwrap(URL(
            string: "https://media.example.invalid/video.mp4"
        ))
        let client = NodeProviderStubHTTPClient { request in
            if request.url.path.hasSuffix("/init") {
                return HTTPResponse(
            ...
            if request.url.host == mediaURL.host {
                ...
                return HTTPResponse(
                    ...
                    statusCode: 200,
                    headers: ["Content-Type": "application/json"],
                    body: Data(#"{"errno":-6,"message":"login required"}"#.utf8)
                )
            }
            return HTTPResponse(
                url: request.url,
                statusCode: 200,
                headers: ["Content-Type": "application/json"],
                body: Data(#"{"url":"https://media.example.invalid/video.mp4","parse":0}"#.utf8)
            )
        }
        let provider = try NodeHTTPSpiderSiteProvider(
            site: SiteConfiguration(
        ...
        do {
            _ = try await provider.player(flag: "百度", episodeURL: "opaque")
            XCTFail("JSON 错误页不得作为媒体直链返回")
        } catch {
            XCTAssertTrue(error.localizedDescription.contains("媒体准备失败"))
        }
    }

    func testStrictNodeMediaReadinessAcceptsNonemptyRangeBytes() async throws {
        let client = NodeProviderStubHTTPClient { request in
            if request.url.path.hasSuffix("/init") {
                return HTTPResponse(
                    url: request.url,
            ...
            if request.url.host == "media.example.invalid" {
                let readiness = request.headers["Range"] == "bytes=0-65535"
                return HTTPResponse(
                    url: request.url,
                    statusCode: 206,
                    headers: [
                        "Content-Type": "video/mp4",
                        "Content-Range": readiness
                            ? "bytes 0-31/1024"
                            : "bytes 0-0/1024"
                    ],
                    body: readiness ? Data(repeating: 1, count: 32) : Data([1])
                )
            }
            return HTTPResponse(
                url: request.url,
        ...
        XCTAssertEqual(result.url, "https://media.example.invalid/video.mp4")
    }
