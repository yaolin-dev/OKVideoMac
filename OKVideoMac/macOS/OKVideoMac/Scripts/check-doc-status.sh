...
assert_contains "$ROOT_README" "upcoming release candidate is **${VERSION} (Build ${BUILD})**"
assert_contains "$ROOT_README_ZH" "即将发布的候选版本是 **${VERSION}（Build ${BUILD}）**"
assert_exact_line "$CHANGELOG" "## [${VERSION}] - 2026-09-05"
assert_contains "$NOTICES" "OKVideoMac ${VERSION} (Build ${BUILD})"
assert_contains "$README" "- Xcode：584 total / 582 passed / 2 intentionally skipped / 0 failed"
