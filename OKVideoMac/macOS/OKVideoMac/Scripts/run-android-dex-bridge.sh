...
"$ADB" -s "$DEVICE" forward --remove tcp:9978 >/dev/null 2>&1 || true
"$ADB" -s "$DEVICE" forward tcp:9978 tcp:9978
"$ADB" -s "$DEVICE" forward --remove tcp:16677 >/dev/null 2>&1 || true
"$ADB" -s "$DEVICE" forward tcp:16677 tcp:6677
curl --fail --silent --show-error --max-time 5 http://127.0.0.1:9978/health
