...
- Build 98 包含 API 24 旧镜像无法通过现代 boot property 接收新生成
...
- 主导航使用 AppKit 原生 Source List、Sidebar 材质、大号原生搜索框和系统语义
  蓝色图标；选中行保持中性灰并正确跟随窗口激活状态。
- 搜索框有文字时第一次 Esc 只清空文字且保持焦点；空框再次 Esc 才退出搜索。
...
- 本机 API 35 私有 ADB A/B 可从 `offline` 转为 `device`，且 Build 98
...
selection, and malformed private key files. Build 98 includes the Emulator's
...
Build 98 also moves the primary navigation to AppKit's native source-list and
sidebar material. Its search field now follows the App Store Escape sequence:
clear a non-empty query while retaining focus, then exit search on the next
Escape once the field is empty.
